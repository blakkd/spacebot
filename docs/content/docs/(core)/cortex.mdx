...
The bulletin also does **not** fan out to live external knowledge sources in this wave. `knowledge_recall` runs on branches when a conversation needs QMD or future adapter-backed evidence. Cortex and bulletin generation stay on the durable-memory side until the retrieval plane is verified and a later design explicitly promotes stable external findings into ambient context.
