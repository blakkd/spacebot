...
Memory recall is always delegated away from the channel. Branches own `memory_recall` and curate what comes back before the channel answers. No user-facing process dumps raw database results directly into the channel context.
...
When the branch needs external or document-oriented evidence instead of durable typed memory, it uses `knowledge_recall` instead. That keeps the boundary clean:

- `memory_recall` is for typed memories stored in Spacebot's native memory graph.
- `knowledge_recall` is for normalized evidence from the native graph plus adapter-backed external sources such as QMD.
