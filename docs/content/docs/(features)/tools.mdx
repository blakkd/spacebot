...
| `knowledge_recall` | Retrieve normalized evidence from native memory and adapter-backed external sources | Branch |
...
│   knowledge_recall (native retrieval plane)  │
...
Branch isolation ensures `memory_recall`, `knowledge_recall`, `channel_recall`, `spacebot_docs`, and `email_search` are never visible to the channel. All tools are registered at creation and live for the lifetime of the branch.
...
// Per branch spawn — creates an isolated ToolServer with memory, knowledge, recall,
// task-board, and optional worker-handoff tools
create_branch_tool_server(...) -> ToolServerHandle
...
`memory_save`, `memory_recall`, `knowledge_recall`, `memory_delete`, `channel_recall`, `spacebot_docs`, `email_search`, and task-board tools on branch ToolServers. `shell`, `file`, `exec`, `set_status`, and connected MCP tools on worker ToolServers. `memory_save` on cortex and compactor ToolServers. These are registered before `.run()` via the builder pattern and live for the lifetime of the ToolServer.
...
Branch and worker ToolServers are created when the process spawns and dropped when it finishes. Each branch gets `memory_save` + `memory_recall` + `knowledge_recall` + `channel_recall` + `spacebot_docs` + `email_search` (plus task board tools). Each worker gets `shell`, `file`, `exec`, `set_status` (bound to that worker's ID), and optionally `browser`, `web_search`, and connected `mcp_*` tools.
...
`spawn_worker` also supports a `task_preset` field for higher-level task shaping without exposing new raw tools. The first preset is `retrieval`, which tells the worker to search worker-only knowledge tools (such as configured `mcp_*` sources) and return synthesized evidence with provenance instead of a raw tool dump.
...
Native retrieval entrypoint for branch-visible evidence gathering across multiple knowledge sources.

- `memory_recall` remains the typed durable-memory tool.
- `knowledge_recall` returns normalized `KnowledgeHit` results with mandatory provenance.
- Today it supports native memory plus QMD through a native adapter contract.
- Google Workspace family descriptors are modeled behind the same branch UX, but their bridge adapter remains normalization-only for now.

Branches use `knowledge_recall` when they need document or external-source evidence. Channels still never see raw MCP output, raw QMD tool names, or bridge-specific command semantics.
